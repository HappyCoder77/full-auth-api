...
class OrderListCreateAPIView(ListCreateAPIView):
    ...
    def handle_exception(self, exc):
        ...
        if isinstance(exc, DRFValidationError):
            if "collection" in exc.detail:
                if exc.detail["collection"][0].code == "required":
                    return Response(
                        {"detail": "El campo collection no puede estar vacío"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                return Response(
                    {"detail": "No existe ninguna colección con el id suministrado"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
