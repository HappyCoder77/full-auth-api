...
class PromotionViewSetTestCase(TestCase):
    ...
    def test_retrieve_promotion(self):
        detail_url = reverse("promotion-detail", kwargs={"pk": self.past_promotion.pk})
    ...
    def test_update_promotion_forbidden(self):
        ...
        response = self.client.patch(partial_update_url, data=data, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_not_found_promotion(self):
        detail_url = reverse("promotion-detail", kwargs={"pk": 10000})
        response = self.client.get(detail_url)
